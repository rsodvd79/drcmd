...
- `help` mostra i comandi disponibili, `history` stampa la cronologia, `exit`/`quit` chiude la shell.
- Il prompt mostra utente/host e directory corrente per ricordare dove stai lavorando.
...
- Modello: `OLLAMA_MODEL` (default `gemma3:1b`) oppure `--model/-m`.
- Endpoint: `OLLAMA_ENDPOINT` (default `http://localhost:11434`) oppure `--endpoint/-e`.
